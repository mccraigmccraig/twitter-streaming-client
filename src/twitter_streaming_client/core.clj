...
(defaction- record-failure-action 
  ...
  (let [last-backoff-ms  (or (:last-backoff-ms twitter-stream) 10000)
    ...
    (start-twitter-stream twitter-stream-agent :force? false)
    (assoc twitter-stream
      :response nil
...
(defn retrieve-queues
  "empty the queues on the twitter-stream and return it"
  [twitter-stream-agent]
  (let [qp (promise)]
...
(defn create-twitter-stream 
  ...
  [method & other-params]
  (let [twitter-stream-agent (agent (TwitterStream. :stopped {} nil nil nil nil)
